...
func main(){
  ...
  defer db.Close()
  var c1, c2 int64
  if err := db.QueryRow("SELECT COUNT(*) FROM event_logs").Scan(&c1); err != nil { fmt.Println("event_logs err:", err); return }
  if err := db.QueryRow("SELECT COUNT(*) FROM event_stats_hourly").Scan(&c2); err != nil { fmt.Println("event_stats_hourly err:", err); return }
